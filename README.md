...
Either way, reviewing, printing, publishing, and voice-editing photos already in `captures/` is exactly the same code and behavior — see [Home Screen: Camera Mode vs FTP Mode](docs/home-screen-modes.md). If you run this on more than one physical device (e.g. one camera, one FTP receiver), each gets its own clone, venv, `secrets.sh`, and `settings.json` — see [Setting Up the Python Virtual Environment](docs/venv-setup.md).
