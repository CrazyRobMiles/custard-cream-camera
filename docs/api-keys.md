...
If you run this app on more than one device (e.g. one in camera mode, one in FTP mode), each device reads its own `secrets.sh` and has its own environment — even if both publish to the same accounts, the keys aren't shared between devices just because the repo is.
