...
`custard_cream_camera.py` never opens a browser itself - if one appears (often along with a prompt to unlock/create a login keyring) right as the app exits, it's almost always a leftover browser window from the one-time `setup_flickr_auth.py` step (see [Publishing to Flickr](publishing-flickr.md)), still sitting open behind the fullscreen camera app the whole time it was running. Closing the app's fullscreen window just reveals what was underneath, it isn't spawning anything new.

Close the stray browser (`pkill chromium`, or whatever browser you used) to clear it. To stop it happening again:

* Run `setup_flickr_auth.py`'s browser step on a different device (phone, laptop) rather than the Pi's own desktop, and close that browser window once you've pasted the verification code back into the terminal.
