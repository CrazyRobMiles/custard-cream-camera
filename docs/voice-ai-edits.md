...
6. **Test it.** [Play mode](capture-and-play-modes.md) has no option to take a fresh photo, so press **Click** first (in Capture mode) if you haven't taken any yet. Then: launch the app, tap **Play**, hold **Speak**, say something like "make it look like a watercolor painting" (if using Vosk, watch the recognised text build up on screen as you talk), and release. The edited image should then appear on screen and save to `captures/ai_<timestamp>.jpg`.
