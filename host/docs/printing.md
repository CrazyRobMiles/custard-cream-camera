...
The **Print** button (see [Reviewing Received Photos](reviewing-photos.md)) sends the currently displayed photo to CUPS via [pycups](https://github.com/OpenPrinting/pycups) — see the [Canon SELPHY CP400 setup](printer-cups-setup.md) for getting a printer configured. Submitting and watching the job runs on a background thread, the same way [publishing](../../lib/publishers/) and [AI edits](voice-ai-edits.md) do, so the app stays responsive while it works. This whole flow (`prepare_print_copy()`/`run_print()`/`wait_for_print_job()`/...) lives in [lib/review_station.py](../../lib/review_station.py), shared unchanged with the [camera app](../../camera/).
...
Set `"test_mode": true` under `"printing"` in [settings.json](../settings.json) to try out the print pipeline — watermark and date stamp included — without spending paper/ink: pressing **Print** saves the exact image that would have been sent to the printer into `"test_folder"` (default `print_tests/`, gitignored) instead of actually printing it. Set back to `false` (the default) to resume printing for real.
...
* `"file"` — path to a PNG, relative to this app's own folder (`host/`, not the repo root - each app has its own `assets/images/watermark.png`). Transparent areas show the photo through, so the source PNG needs an alpha channel.
...
* `"format"` — a [`strftime`](https://docs.python.org/3/library/time.html#time.strftime) format string. The stamp reflects the received file's modification time - the moment it was saved into `captures/` after arriving over FTP, not when the camera actually took the shot (the camera's own EXIF capture time isn't read).
...
Both default to the bottom corners (watermark bottom-right, date stamp bottom-left) so they don't overlap. If a watermark/date stamp fails to apply for any reason (bad path, corrupt font, etc.), that step is skipped with a printed error rather than blocking the print entirely.
